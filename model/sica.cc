...
//////////////////////NotifyRxDropped 
void 
Sica::NotifyRxDropped (Ptr<const Packet> packet,double snr)
{
...
Sica::InitializeTimers()
 {
  ...
  // TMax=MilliSeconds(static_cast <uint64_t>(Time::ToDouble(HelloInterval,(Time::Unit)1)/Max_CH));
...
Sica::EndSenseCurrentChannel()
{
  ...
  m_channelSenseFlag=false;
  double bx;
  double tBusy=Time::ToDouble(m_busyChTime, (Time::Unit)1);
  double tIdle=Time::ToDouble(m_idleChTime,(Time::Unit)1);
...
Sica::SendPacket(Ptr<Packet> packet,Ptr <NetDevice> device , uint32_t protocolNumber)
{
  ...
  if (protocolNumber==SICA_HELLO_PORT)
    {
      UniformVariable uniRnd(0,Time::ToDouble(m_bcastSendDelay,(Time::Unit)3));
...
Sica::ComputeStageLoss(uint32_t c)
{
  ...
  double TH=Time::ToDouble(HelloInterval,Time::S);
  double Ds;
  double Loss;
  double ML;
 if (m_rChannel == c)
   ...
   Ds=Time::ToDouble(SwitchingDelay,Time::S);
       if (rNi!=0 && b!=0 && TH!=0)
	 {
